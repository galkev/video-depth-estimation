...
class ResNeXtBottleneckDecode(nn.Module):
    expansion = 2

    def __init__(self, outplanes, planes, cardinality, stride=1, downsample=None):
        super(ResNeXtBottleneckDecode, self).__init__()
        #inplanes = outplanes * self.expansion
...
class ResNeXtDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # torch.Size([1, 3, 16, 224, 224])  <- input
        # torch.Size([1, 64, 16, 112, 112])
        # torch.Size([1, 64, 16, 112, 112])
        # torch.Size([1, 64, 16, 112, 112])
        # torch.Size([1, 64, 8, 56, 56])
        # torch.Size([1, 256, 8, 56, 56])
        # torch.Size([1, 512, 4, 28, 28])
        # torch.Size([1, 1024, 2, 14, 14])
        # torch.Size([1, 2048, 1, 7, 7])  <- output
...
def resnet101_decoder(**kwargs):
    ...
    """
    model = ResNeXtDecoder(ResNeXtBottleneckDecode, [3, 4, 23, 3], **kwargs)
    return model
