...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.add_module('conv_1', nn.Conv3d(num_input_features, bn_size * growth_rate,
...
class DenseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.features:
            # in_size = features.size()
            if type(layer) == _Transition:
                # save for skip connection before every transition layer
                ccx.append(features)
            features = layer(features)
            # out_size = features.size()

            #print(type(layer), "in:", in_size, "out:", out_size)
